...
let twDst0 = 0, twDst = 0, twExpPlay = Math.log2(1.25);
...
const zoomCanvas = (x = cnvX, y = cnvY, scaleExp = 0, announcement = false) => {
    ...
    scaleCanvas(2 ** Math.floor(cnvScaleExp = scaleExp), announcement);
...
const stretchCanvas = (x = cnvX, y = cnvY, dltExp = 0, announcement = false) => {
    ...
    zoomCanvas(x, y, cnvScaleExp, announcement);
}
...
const initPointers = event => {
    ...
    // パラメーターの初期化
    trajectories = [];
    twX0 = twY0 = twX = twY = twAglB = twAgl0 = twAgl = twDst0 = twDst = NaN;
