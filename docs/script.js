...
const computeCanvasAxis = () => {
    cnvXAxisX = Math.cos(cnvAngle * Math.PI / 180);
    cnvXAxisY = Math.sin(cnvAngle * Math.PI / 180);
    cnvYAxisX = Math.cos((cnvAngle + 90) * Math.PI / 180);
    cnvYAxisY = Math.sin((cnvAngle + 90) * Math.PI / 180);
};
        ...
        case 2:
            if (mainPointerId && subPointerId === event.pointerId) {
                ...
                if (mainPointer) {
                    ...
                    const dltX = twX - twX0;
                    const dltY = twY - twY0;
                    const sclDst = twDst === 0 ? 0 : twDst / twDst0;
