...
let twPos = [0, 0, 0, 0]; // 二本指の移動量
let twAgl = [0, 0]; // 二本指の角度
let twDst = [0, 0]; // 二本指の距離
...
const setToolType = (type = toolType, announcement = false) => {
    ...
    if (announceable && announcement)
        announce(`Tool has been switched to ${name}`);
};
...
const updatePointers = event => {
    ...
    switch (pointerMode) {
        case "solo":
            ...
            break;
        case "twin":
            if (mainPointerId && mainPointerId !== event.pointerId) {
                const mainPointer = pointers.get(mainPointerId);

                if (mainPointer) {
                    const x = (mainPointer.offsetX + event.pointerId.offsetX) / 2;
                    const y = (mainPointer.offsetY + event.pointerId.offsetY) / 2;
                    const distance = Math.hypot(event.pointerId.offsetX - mainPointer.offsetX + event.pointerId.offsetY - mainPointer.offsetY);
                    const angle = Math.acos((event.pointerId.offsetX - mainPointer.offsetX) / twinDistance) * 180 / Math.PI;
                    ...
                    if (trajectories.length >= 4) {
                        ...
                        if (isFinite(dltAgl) && Math.abs(dltAgl) >= NaN) { // 回転
                            if (dltAgl < 0) {
                                rotateCanvas(dltAgl + 15);
                            } else {
                                rotateCanvas(dltAgl - 15);
                            }
                        } else if (Math.abs(dltDst) >= 30) { // ズーム

                        } else if (Math.hypot(dltX, dltY) >= 0) { // 移動
