...
const TO_DEGS = 180 / Math.PI;

const clamp = (value = 0, min = 0, max = 0) => {
    return min > max ? clamp(value, max, min) :  Math.max(min, Math.min(value, max));
};
...
const play = (value = 0, play = 0) => {
    return value >= 0 ? Math.max(0, value - play) : Math.min(value + play, 0);
}
...
const calcStroke = (vertices = []) => {
    ...
    for (let i = 0; i < length; i += 2) {
        stroke.push(...calcSegment(Math.ceil(vertices[i]), Math.ceil(vertices[i + 1]), Math.ceil(vertices[i + 2]), Math.ceil(vertices[i + 3])));
    }
